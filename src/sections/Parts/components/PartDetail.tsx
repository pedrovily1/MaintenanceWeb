...
export const PartDetail = ({ partId }: PartDetailProps) => {
  ...
  // Work orders that used this part
  ...
  // All WOs where this part was consumed (for history tab)
  ...
  const handleRestock = () => {
    ...
    const loc = part.inventory.find(i => i.locationId === restockLocId);
    restockPart(part.id, restockLocId, loc?.locationName ?? restockLocId, restockQty);
                            ...
                            return (
                              <tr key={inv.locationId} className="border-b border-[var(--border)] last:border-0 hover:bg-gray-50 transition-colors">
                                ...
                                <td className="px-3 py-2 text-center">
                                  {low ? (
                                    <span className="bg-amber-100 text-amber-700 text-[9px] font-bold uppercase tracking-wide px-1.5 py-0.5 rounded">
                                      Low
                                    </span>
                                  ) : (
                                    <span className="bg-green-50 text-green-700 text-[9px] font-bold uppercase tracking-wide px-1.5 py-0.5 rounded">
                                      OK
                                    </span>
                                  )}
                                </td>
                ...
                >
                  ...
                  {part.inventory.map(inv => (
                    <option key={inv.locationId} value={inv.locationId}>
                      {inv.locationName} (current: {inv.quantity})
                    </option>
                  ))}
                </select>
              </div>
