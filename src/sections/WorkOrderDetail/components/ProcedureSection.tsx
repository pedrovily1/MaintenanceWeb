export const ProcedureSection = () => {
  return (
    <div className="box-border caret-transparent shrink-0 max-w-[848px] mt-4">
      <div className="box-border caret-transparent shrink-0 max-w-[848px] my-4">
        <div className="text-white bg-accent box-border caret-transparent shrink-0 p-6 rounded-bl rounded-br">
          <div className="box-border caret-transparent flex shrink-0 justify-between">
            <h2 className="text-[21px] font-semibold box-border caret-transparent basis-[0%] grow leading-[25.2px] break-words">
              Weekly maintenance work order Template
            </h2>
          </div>
          <div className="box-border caret-transparent shrink-0 leading-[21px] break-words mt-2"></div>
        </div>
        <div className="border-b-zinc-200 border-l-neutral-800 border-r-neutral-800 border-t-neutral-800 box-border caret-transparent shrink-0 mx-6 py-6 border-b">
          <h4 className="text-[15.4px] font-medium box-border caret-transparent shrink-0 leading-[23.1px]">
            Table of Contents:
          </h4>
          <ol className="box-border caret-transparent shrink-0 list-decimal pl-0">
            <li className="relative box-border caret-transparent block shrink-0 pl-4 pt-2 before:accent-auto before:box-border before:caret-transparent before:text-neutral-800 before:block before:shrink-0 before:text-sm before:not-italic before:normal-nums before:font-normal before:tracking-[normal] before:leading-[16.8px] before:list-outside before:list-decimal before:pointer-events-auto before:absolute before:right-[calc(100%_-_20px)] before:text-right before:indent-[0px] before:normal-case before:visible before:pr-2 before:border-separate before:top-2 before:font-system_ui">
              <div className="items-center box-border caret-transparent flex basis-[0%] grow">
                <span className="box-border caret-transparent block shrink-0 w-5"></span>
                <a
                  href="https://app.getmaintainx.com/workorders/84658821#procedure-884759851-Work_Accomplished"
                  className="text-accent box-border caret-transparent block basis-[0%] grow break-words text-ellipsis text-nowrap overflow-hidden hover:text-accent-hover hover:border-accent-hover"
                >
                  Work Accomplished
                </a>
              </div>
            </li>
            <li className="relative box-border caret-transparent block shrink-0 pl-4 pt-2 before:accent-auto before:box-border before:caret-transparent before:text-neutral-800 before:block before:shrink-0 before:text-sm before:not-italic before:normal-nums before:font-normal before:tracking-[normal] before:leading-[16.8px] before:list-outside before:list-decimal before:pointer-events-auto before:absolute before:right-[calc(100%_-_20px)] before:text-right before:indent-[0px] before:normal-case before:visible before:pr-2 before:border-separate before:top-2 before:font-system_ui">
              <div className="items-center box-border caret-transparent flex basis-[0%] grow">
                <span className="box-border caret-transparent block shrink-0 w-5"></span>
                <a
                  href="https://app.getmaintainx.com/workorders/84658821#procedure-884759870-Issues_or_Problems"
                  className="text-accent box-border caret-transparent block basis-[0%] grow break-words text-ellipsis text-nowrap overflow-hidden hover:text-accent-hover hover:border-accent-hover"
                >
                  Issues or Problems
                </a>
              </div>
            </li>
            <li className="relative box-border caret-transparent block shrink-0 pl-4 pt-2 before:accent-auto before:box-border before:caret-transparent before:text-neutral-800 before:block before:shrink-0 before:text-sm before:not-italic before:normal-nums before:font-normal before:tracking-[normal] before:leading-[16.8px] before:list-outside before:list-decimal before:pointer-events-auto before:absolute before:right-[calc(100%_-_20px)] before:text-right before:indent-[0px] before:normal-case before:visible before:pr-2 before:border-separate before:top-2 before:font-system_ui">
              <div className="items-center box-border caret-transparent flex basis-[0%] grow">
                <span className="box-border caret-transparent block shrink-0 w-5"></span>
                <a
                  href="https://app.getmaintainx.com/workorders/84658821#procedure-884759875-Outstanding_Items_or_Customer_Comments"
                  className="text-accent box-border caret-transparent block basis-[0%] grow break-words text-ellipsis text-nowrap overflow-hidden hover:text-accent-hover hover:border-accent-hover"
                >
                  Outstanding Items or Customer Comments:
                </a>
              </div>
            </li>
            <li className="relative box-border caret-transparent block shrink-0 pl-4 pt-2 before:accent-auto before:box-border before:caret-transparent before:text-neutral-800 before:block before:shrink-0 before:text-sm before:not-italic before:normal-nums before:font-normal before:tracking-[normal] before:leading-[16.8px] before:list-outside before:list-decimal before:pointer-events-auto before:absolute before:right-[calc(100%_-_20px)] before:text-right before:indent-[0px] before:normal-case before:visible before:pr-2 before:border-separate before:top-2 before:font-system_ui">
              <div className="items-center box-border caret-transparent flex basis-[0%] grow">
                <span className="box-border caret-transparent block shrink-0 w-5"></span>
                <a
                  href="https://app.getmaintainx.com/workorders/84658821#procedure-884759878-Review_Sign"
                  className="text-accent box-border caret-transparent block basis-[0%] grow break-words text-ellipsis text-nowrap overflow-hidden hover:text-accent-hover hover:border-accent-hover"
                >
                  Review &amp; Sign
                </a>
              </div>
            </li>
          </ol>
        </div>
        <div className="box-border caret-transparent shrink-0 mt-2 mx-6">
          <h2 className="text-[21px] font-semibold items-center box-border caret-transparent flex shrink-0 leading-[25.2px] break-words mt-6">
            <img
              src="https://c.animaapp.com/mkof8zon8iICvl/assets/icon-52.svg"
              alt="Icon"
              className="box-border caret-transparent shrink-0 h-[7px] w-3 -scale-100"
            />
            <span className="box-border caret-transparent block basis-[0%] grow break-words ml-2">
              Work Accomplished
            </span>
          </h2>
          <div className="box-border caret-transparent shrink-0 h-[3828px]">
            <div className="box-border caret-transparent flex flex-col shrink-0">
              <div className="box-border caret-transparent shrink-0">
                <div className="relative box-border caret-transparent grow border border-[var(--border)] mt-4 rounded-bl rounded-br rounded-tl rounded-tr border-solid">
                  <div className="box-border caret-transparent flex shrink-0 justify-start">
                    <div className="box-border caret-transparent grow-[4] w-full p-4">
                      <div className="items-center box-border caret-transparent flex shrink-0 justify-start">
                        <div className="box-border caret-transparent w-full">
                          <label className="box-border caret-transparent shrink-0 break-words">
                            Generator Checklist
                          </label>
                        </div>
                      </div>
                      <div className="absolute box-border caret-transparent shrink-0 right-2 top-2"></div>
                      <div className="box-border caret-transparent shrink-0 mt-4">
                        <ul className="box-border caret-transparent shrink-0 list-none mt-3.5 pl-0">
                          <li className="box-border caret-transparent shrink-0">
                            <label className="items-center box-border caret-transparent flex shrink-0">
                              <input
                                type="checkbox"
                                value="Visual Inspection"
                                className="relative text-neutral-600 bg-accent box-border caret-transparent block shrink-0 h-4 w-4 border border-accent p-0 rounded-bl rounded-br rounded-tl rounded-tr border-solid before:accent-auto before:box-border before:caret-transparent before:text-neutral-600 before:block before:shrink-0 before:text-sm before:not-italic before:normal-nums before:font-normal before:h-[9px] before:tracking-[normal] before:leading-[16.8px] before:list-outside before:list-none before:pointer-events-auto before:absolute before:text-start before:indent-[0px] before:normal-case before:rotate-45 before:visible before:w-1 before:border-r-2 before:border-b-2 before:border-separate before:border-[var(--border)] before:left-[5px] before:top-px before:font-system_ui"
                              />
                              <span className="box-border caret-transparent block grow break-words w-0 ml-2">
                                Visual Inspection
                              </span>
                            </label>
                          </li>
                          <li className="box-border caret-transparent shrink-0 mt-4">
                            <label className="items-center box-border caret-transparent flex shrink-0">
                              <input
                                type="checkbox"
                                value="Check Oil Level"
                                className="relative text-neutral-600 bg-accent box-border caret-transparent block shrink-0 h-4 w-4 border border-accent p-0 rounded-bl rounded-br rounded-tl rounded-tr border-solid before:accent-auto before:box-border before:caret-transparent before:text-neutral-600 before:block before:shrink-0 before:text-sm before:not-italic before:normal-nums before:font-normal before:h-[9px] before:tracking-[normal] before:leading-[16.8px] before:list-outside before:list-none before:pointer-events-auto before:absolute before:text-start before:indent-[0px] before:normal-case before:rotate-45 before:visible before:w-1 before:border-r-2 before:border-b-2 before:border-separate before:border-[var(--border)] before:left-[5px] before:top-px before:font-system_ui"
                              />
                              <span className="box-border caret-transparent block grow break-words w-0 ml-2">
                                Check Oil Level
                              </span>
                            </label>
                          </li>
                          <li className="box-border caret-transparent shrink-0 mt-4">
                            <label className="items-center box-border caret-transparent flex shrink-0">
                              <input
                                type="checkbox"
                                value="Check Coolant Level"
                                className="relative text-neutral-600 bg-accent box-border caret-transparent block shrink-0 h-4 w-4 border border-accent p-0 rounded-bl rounded-br rounded-tl rounded-tr border-solid before:accent-auto before:box-border before:caret-transparent before:text-neutral-600 before:block before:shrink-0 before:text-sm before:not-italic before:normal-nums before:font-normal before:h-[9px] before:tracking-[normal] before:leading-[16.8px] before:list-outside before:list-none before:pointer-events-auto before:absolute before:text-start before:indent-[0px] before:normal-case before:rotate-45 before:visible before:w-1 before:border-r-2 before:border-b-2 before:border-separate before:border-[var(--border)] before:left-[5px] before:top-px before:font-system_ui"
                              />
                              <span className="box-border caret-transparent block grow break-words w-0 ml-2">
                                Check Coolant Level
                              </span>
                            </label>
                          </li>
                          <li className="box-border caret-transparent shrink-0 mt-4">
                            <label className="items-center box-border caret-transparent flex shrink-0">
                              <input
                                type="checkbox"
                                value="Check Fuel"
                                className="relative text-neutral-600 bg-accent box-border caret-transparent block shrink-0 h-4 w-4 border border-accent p-0 rounded-bl rounded-br rounded-tl rounded-tr border-solid before:accent-auto before:box-border before:caret-transparent before:text-neutral-600 before:block before:shrink-0 before:text-sm before:not-italic before:normal-nums before:font-normal before:h-[9px] before:tracking-[normal] before:leading-[16.8px] before:list-outside before:list-none before:pointer-events-auto before:absolute before:text-start before:indent-[0px] before:normal-case before:rotate-45 before:visible before:w-1 before:border-r-2 before:border-b-2 before:border-separate before:border-[var(--border)] before:left-[5px] before:top-px before:font-system_ui"
                              />
                              <span className="box-border caret-transparent block grow break-words w-0 ml-2">
                                Check Fuel
                              </span>
                            </label>
                          </li>
                          <li className="box-border caret-transparent shrink-0 mt-4">
                            <label className="items-center box-border caret-transparent flex shrink-0">
                              <input
                                type="checkbox"
                                value="Check Air Filter"
                                className="relative text-neutral-600 bg-accent box-border caret-transparent block shrink-0 h-4 w-4 border border-accent p-0 rounded-bl rounded-br rounded-tl rounded-tr border-solid before:accent-auto before:box-border before:caret-transparent before:text-neutral-600 before:block before:shrink-0 before:text-sm before:not-italic before:normal-nums before:font-normal before:h-[9px] before:tracking-[normal] before:leading-[16.8px] before:list-outside before:list-none before:pointer-events-auto before:absolute before:text-start before:indent-[0px] before:normal-case before:rotate-45 before:visible before:w-1 before:border-r-2 before:border-b-2 before:border-separate before:border-[var(--border)] before:left-[5px] before:top-px before:font-system_ui"
                              />
                              <span className="box-border caret-transparent block grow break-words w-0 ml-2">
                                Check Air Filter
                              </span>
                            </label>
                          </li>
                          <li className="box-border caret-transparent shrink-0 mt-4">
                            <label className="items-center box-border caret-transparent flex shrink-0">
                              <input
                                type="checkbox"
                                value="Check Battery Connections/Corrosion"
                                className="relative text-neutral-600 bg-accent box-border caret-transparent block shrink-0 h-4 w-4 border border-accent p-0 rounded-bl rounded-br rounded-tl rounded-tr border-solid before:accent-auto before:box-border before:caret-transparent before:text-neutral-600 before:block before:shrink-0 before:text-sm before:not-italic before:normal-nums before:font-normal before:h-[9px] before:tracking-[normal] before:leading-[16.8px] before:list-outside before:list-none before:pointer-events-auto before:absolute before:text-start before:indent-[0px] before:normal-case before:rotate-45 before:visible before:w-1 before:border-r-2 before:border-b-2 before:border-separate before:border-[var(--border)] before:left-[5px] before:top-px before:font-system_ui"
                              />
                              <span className="box-border caret-transparent block grow break-words w-0 ml-2">
                                Check Battery Connections/Corrosion
                              </span>
                            </label>
                          </li>
                          <li className="box-border caret-transparent shrink-0 mt-4">
                            <label className="items-center box-border caret-transparent flex shrink-0">
                              <input
                                type="checkbox"
                                value="Check Connections for fraying"
                                className="relative text-neutral-600 bg-accent box-border caret-transparent block shrink-0 h-4 w-4 border border-accent p-0 rounded-bl rounded-br rounded-tl rounded-tr border-solid before:accent-auto before:box-border before:caret-transparent before:text-neutral-600 before:block before:shrink-0 before:text-sm before:not-italic before:normal-nums before:font-normal before:h-[9px] before:tracking-[normal] before:leading-[16.8px] before:list-outside before:list-none before:pointer-events-auto before:absolute before:text-start before:indent-[0px] before:normal-case before:rotate-45 before:visible before:w-1 before:border-r-2 before:border-b-2 before:border-separate before:border-[var(--border)] before:left-[5px] before:top-px before:font-system_ui"
                              />
                              <span className="box-border caret-transparent block grow break-words w-0 ml-2">
                                Check Connections for fraying
                              </span>
                            </label>
                          </li>
                          <li className="box-border caret-transparent shrink-0 mt-4">
                            <label className="items-center box-border caret-transparent flex shrink-0">
                              <input
                                type="checkbox"
                                value="Check Fan belts"
                                className="relative text-neutral-600 bg-accent box-border caret-transparent block shrink-0 h-4 w-4 border border-accent p-0 rounded-bl rounded-br rounded-tl rounded-tr border-solid before:accent-auto before:box-border before:caret-transparent before:text-neutral-600 before:block before:shrink-0 before:text-sm before:not-italic before:normal-nums before:font-normal before:h-[9px] before:tracking-[normal] before:leading-[16.8px] before:list-outside before:list-none before:pointer-events-auto before:absolute before:text-start before:indent-[0px] before:normal-case before:rotate-45 before:visible before:w-1 before:border-r-2 before:border-b-2 before:border-separate before:border-[var(--border)] before:left-[5px] before:top-px before:font-system_ui"
                              />
                              <span className="box-border caret-transparent block grow break-words w-0 ml-2">
                                Check Fan belts
                              </span>
                            </label>
                          </li>
                          <li className="box-border caret-transparent shrink-0 mt-4">
                            <label className="items-center box-border caret-transparent flex shrink-0">
                              <input
                                type="checkbox"
                                value="Check Hours"
                                className="relative text-neutral-600 bg-accent box-border caret-transparent block shrink-0 h-4 w-4 border border-accent p-0 rounded-bl rounded-br rounded-tl rounded-tr border-solid before:accent-auto before:box-border before:caret-transparent before:text-neutral-600 before:block before:shrink-0 before:text-sm before:not-italic before:normal-nums before:font-normal before:h-[9px] before:tracking-[normal] before:leading-[16.8px] before:list-outside before:list-none before:pointer-events-auto before:absolute before:text-start before:indent-[0px] before:normal-case before:rotate-45 before:visible before:w-1 before:border-r-2 before:border-b-2 before:border-separate before:border-[var(--border)] before:left-[5px] before:top-px before:font-system_ui"
                              />
                              <span className="box-border caret-transparent block grow break-words w-0 ml-2">
                                Check Hours
                              </span>
                            </label>
                          </li>
                          <li className="box-border caret-transparent shrink-0 mt-4">
                            <label className="items-center box-border caret-transparent flex shrink-0">
                              <input
                                type="checkbox"
                                value="Ensure generator runs shortly (5 min) for general start and shutdown check."
                                className="relative text-neutral-600 bg-accent box-border caret-transparent block shrink-0 h-4 w-4 border border-accent p-0 rounded-bl rounded-br rounded-tl rounded-tr border-solid before:accent-auto before:box-border before:caret-transparent before:text-neutral-600 before:block before:shrink-0 before:text-sm before:not-italic before:normal-nums before:font-normal before:h-[9px] before:tracking-[normal] before:leading-[16.8px] before:list-outside before:list-none before:pointer-events-auto before:absolute before:text-start before:indent-[0px] before:normal-case before:rotate-45 before:visible before:w-1 before:border-r-2 before:border-b-2 before:border-separate before:border-[var(--border)] before:left-[5px] before:top-px before:font-system_ui"
                              />
                              <span className="box-border caret-transparent block grow break-words w-0 ml-2">
                                Ensure generator runs shortly (5 min) for
                                general start and shutdown check.
                              </span>
                            </label>
                          </li>
                        </ul>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="relative box-border caret-transparent grow border border-[var(--border)] mt-4 rounded-bl rounded-br rounded-tl rounded-tr border-solid">
                  <div className="box-border caret-transparent flex shrink-0 justify-start">
                    <div className="box-border caret-transparent grow-[4] w-full p-4">
                      <div className="items-center box-border caret-transparent flex shrink-0 justify-start">
                        <div className="box-border caret-transparent w-full">
                          <label className="box-border caret-transparent shrink-0 break-words">
                            Generator Photo of Hours
                          </label>
                        </div>
                      </div>
                      <div className="absolute box-border caret-transparent shrink-0 right-2 top-2"></div>
                      <div className="box-border caret-transparent shrink-0 mt-4">
                        <div className="box-border caret-transparent shrink-0">
                          <div className="relative box-border caret-transparent shrink-0 mt-4">
                            <div className="box-border caret-transparent shrink-0">
                              <div className="font-semibold box-border caret-transparent shrink-0"></div>
                              <div className="box-border caret-transparent shrink-0 mt-2">
                                <button
                                  type="button"
                                  className="text-slate-400 text-[11.2px] font-medium items-center bg-gray-50 caret-transparent flex flex-col shrink-0 justify-center leading-[13.44px] min-h-[100px] text-center w-full border border-[var(--border)] py-6 rounded-bl rounded-br rounded-tl rounded-tr border-dashed"
                                >
                                  <div className="box-border caret-transparent shrink-0 mb-1">
                                    <img
                                      src="https://c.animaapp.com/mkof8zon8iICvl/assets/icon-53.svg"
                                      alt="Icon"
                                      className="box-border caret-transparent shrink-0 h-5 w-5"
                                    />
                                  </div>
                                  Add Pictures/Files
                                </button>
                                <input
                                  type="file"
                                  className="text-neutral-600 items-baseline bg-transparent box-border caret-transparent hidden shrink-0 text-ellipsis text-nowrap p-0"
                                />
                              </div>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="relative box-border caret-transparent grow border border-[var(--border)] mt-4 rounded-bl rounded-br rounded-tl rounded-tr border-solid">
                  <div className="box-border caret-transparent flex shrink-0 justify-start">
                    <div className="box-border caret-transparent grow-[4] w-full p-4">
                      <div className="items-center box-border caret-transparent flex shrink-0 justify-start">
                        <div className="box-border caret-transparent w-full">
                          <label className="box-border caret-transparent shrink-0 break-words">
                            Generator Hours
                          </label>
                        </div>
                      </div>
                      <div className="absolute box-border caret-transparent shrink-0 right-2 top-2"></div>
                      <div className="box-border caret-transparent shrink-0 mt-4">
                        <div className="box-border caret-transparent flex flex-col shrink-0">
                          <span className="box-border caret-transparent block shrink-0 max-w-[400px] text-ellipsis text-nowrap overflow-hidden">
                            <a
                              href="https://app.getmaintainx.com/meters/294355"
                              className="text-accent box-border caret-transparent shrink-0 break-words"
                            >
                              Generator Hours
                            </a>
                          </span>
                          <div className="text-gray-600 text-[11.2px] box-border caret-transparent shrink-0 leading-[13.44px]">
                            Last Reading: 69.1 Hours
                          </div>
                        </div>
                        <div className="box-border caret-transparent shrink-0 mt-2">
                          <div className="relative box-border caret-transparent shrink-0">
                            <input
                              placeholder="Enter Number"
                              type="number"
                              value="69.1"
                              className="bg-gray-50 box-border caret-transparent shrink-0 leading-5 min-h-[38px] w-full border border-[var(--border)] my-1 p-2 rounded-bl rounded-br rounded-tl rounded-tr border-solid"
                            />
                            <span className="absolute text-gray-600 border-b-gray-600 border-l-zinc-200 border-r-gray-600 border-t-gray-600 box-border caret-transparent block shrink-0 leading-[17.5px] pl-2 border-l right-8 top-3.5">
                              Hours
                            </span>
                          </div>
                        </div>
                        <div className="items-center box-border caret-transparent flex shrink-0 mt-4">
                          <div className="box-border caret-transparent shrink-0 -mt-3">
                            <div className="box-border caret-transparent shrink-0 mx-4 py-2">
                              <div className="box-border caret-transparent shrink-0">
                                <button
                                  type="button"
                                  className="text-accent font-medium items-center bg-transparent caret-transparent flex fill-blue-500 shrink-0 justify-center min-h-8 stroke-blue-500 text-center text-nowrap mt-3 pl-0 pr-4 py-[11px] rounded-bl rounded-br rounded-tl rounded-tr hover:text-accent-hover hover:border-accent-hover"
                                >
                                  + Add Pictures/Files
                                </button>
                                <input
                                  type="file"
                                  className="text-neutral-600 items-baseline bg-transparent box-border caret-transparent hidden shrink-0 text-ellipsis text-nowrap p-0"
                                />
                              </div>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="relative box-border caret-transparent grow border border-[var(--border)] mt-4 rounded-bl rounded-br rounded-tl rounded-tr border-solid">
                  <div className="box-border caret-transparent flex shrink-0 justify-start">
                    <div className="box-border caret-transparent grow-[4] w-full p-4">
                      <div className="items-center box-border caret-transparent flex shrink-0 justify-start">
                        <div className="box-border caret-transparent w-full">
                          <label className="box-border caret-transparent shrink-0 break-words">
                            Generator Faults (if applicable)
                          </label>
                        </div>
                      </div>
                      <div className="absolute box-border caret-transparent shrink-0 right-2 top-2"></div>
                      <div className="box-border caret-transparent shrink-0 mt-4">
                        <div className="box-border caret-transparent flex flex-col shrink-0 gap-y-0">
                          <div className="self-start box-border caret-transparent shrink-0"></div>
                          <div className="box-border caret-transparent gap-x-2 flex flex-col shrink-0 gap-y-2">
                            <textarea
                              placeholder="Enter Text"
                              className="bg-transparent box-border caret-transparent flex shrink-0 h-10 tracking-[-0.2px] leading-[20.0004px] max-h-[220px] min-h-10 w-full border-[var(--border)] px-3 py-[9px] rounded-bl rounded-br rounded-tl rounded-tr"
                            >
                              None
                            </textarea>
                          </div>
                        </div>
                        <div className="items-center box-border caret-transparent flex shrink-0"></div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="relative box-border caret-transparent grow border border-[var(--border)] mt-4 rounded-bl rounded-br rounded-tl rounded-tr border-solid">
                  <div className="box-border caret-transparent flex shrink-0 justify-start">
                    <div className="box-border caret-transparent grow-[4] w-full p-4">
                      <div className="items-center box-border caret-transparent flex shrink-0 justify-start">
                        <div className="box-border caret-transparent w-full">
                          <label className="box-border caret-transparent shrink-0 break-words">
                            HVAC - Operational (Software, Hardware, Airflow)
                          </label>
                        </div>
                      </div>
                      <div className="absolute box-border caret-transparent shrink-0 right-2 top-2"></div>
                      <div className="box-border caret-transparent shrink-0 mt-4">
                        <ul className="box-border caret-transparent gap-x-2 block shrink-0 grid-cols-[repeat(3,1fr)] list-none gap-y-2 mt-4 pl-0 md:grid">
                          <li className="box-border caret-transparent flex shrink-0 min-w-0 py-1 md:min-w-[auto] md:py-0">
                            <label className="relative box-border caret-transparent block shrink-0 text-center w-full p-2.5">
                              <input
                                type="radio"
                                value="PASS"
                                className="absolute text-teal-500 bg-emerald-50 box-border caret-transparent block shrink-0 h-full text-start w-full border border-teal-500 p-0 rounded-bl rounded-br rounded-tl rounded-tr border-solid left-0 top-0"
                              />
                              <span className="relative text-teal-500 font-medium box-border caret-transparent shrink-0">
                                Pass
                              </span>
                            </label>
                          </li>
                          <li className="box-border caret-transparent flex shrink-0 min-w-0 py-1 md:min-w-[auto] md:py-0">
                            <label className="relative box-border caret-transparent block shrink-0 text-center w-full p-2.5">
                              <input
                                type="radio"
                                value="FLAG"
                                className="absolute text-neutral-600 bg-transparent box-border caret-transparent block shrink-0 h-full text-start w-full border border-[var(--border)] p-0 rounded-bl rounded-br rounded-tl rounded-tr border-solid left-0 top-0"
                              />
                              <span className="relative text-orange-500 font-medium box-border caret-transparent shrink-0">
                                Flag
                              </span>
                            </label>
                          </li>
                          <li className="box-border caret-transparent flex shrink-0 min-w-0 py-1 md:min-w-[auto] md:py-0">
                            <label className="relative box-border caret-transparent block shrink-0 text-center w-full p-2.5">
                              <input
                                type="radio"
                                value="FAIL"
                                className="absolute text-neutral-600 bg-transparent box-border caret-transparent block shrink-0 h-full text-start w-full border border-[var(--border)] p-0 rounded-bl rounded-br rounded-tl rounded-tr border-solid left-0 top-0"
                              />
                              <span className="relative text-red-500 font-medium box-border caret-transparent shrink-0">
                                Fail
                              </span>
                            </label>
                          </li>
                        </ul>
                        <div className="items-center box-border caret-transparent flex shrink-0 mt-4">
                          <button
                            type="button"
                            className="text-accent font-medium items-center bg-transparent caret-transparent flex fill-blue-500 shrink-0 justify-center min-h-8 stroke-blue-500 text-center text-nowrap mr-4 p-0 rounded-bl rounded-br rounded-tl rounded-tr hover:text-accent-hover hover:border-accent-hover"
                          >
                            + Add notes
                          </button>
                          <div className="box-border caret-transparent shrink-0 -mt-3">
                            <div className="box-border caret-transparent shrink-0 mx-4 py-2">
                              <div className="box-border caret-transparent shrink-0">
                                <button
                                  type="button"
                                  className="text-accent font-medium items-center bg-transparent caret-transparent flex fill-blue-500 shrink-0 justify-center min-h-8 stroke-blue-500 text-center text-nowrap mt-3 pl-0 pr-4 py-[11px] rounded-bl rounded-br rounded-tl rounded-tr hover:text-accent-hover hover:border-accent-hover"
                                >
                                  + Add Pictures/Files
                                </button>
                                <input
                                  type="file"
                                  className="text-neutral-600 items-baseline bg-transparent box-border caret-transparent hidden shrink-0 text-ellipsis text-nowrap p-0"
                                />
                              </div>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="relative box-border caret-transparent grow border border-[var(--border)] mt-4 rounded-bl rounded-br rounded-tl rounded-tr border-solid">
                  <div className="box-border caret-transparent flex shrink-0 justify-start">
                    <div className="box-border caret-transparent grow-[4] w-full p-4">
                      <div className="items-center box-border caret-transparent flex shrink-0 justify-start">
                        <div className="box-border caret-transparent w-full">
                          <label className="box-border caret-transparent shrink-0 break-words">
                            Fire extinguishers
                          </label>
                        </div>
                      </div>
                      <div className="absolute box-border caret-transparent shrink-0 right-2 top-2"></div>
                      <div className="box-border caret-transparent shrink-0 mt-4">
                        <ul className="box-border caret-transparent shrink-0 list-none mt-3.5 pl-0">
                          <li className="box-border caret-transparent shrink-0">
                            <label className="items-center box-border caret-transparent flex shrink-0">
                              <input
                                type="checkbox"
                                value="Ensure they pass fill level and inspection is up to date"
                                className="relative text-neutral-600 bg-accent box-border caret-transparent block shrink-0 h-4 w-4 border border-accent p-0 rounded-bl rounded-br rounded-tl rounded-tr border-solid before:accent-auto before:box-border before:caret-transparent before:text-neutral-600 before:block before:shrink-0 before:text-sm before:not-italic before:normal-nums before:font-normal before:h-[9px] before:tracking-[normal] before:leading-[16.8px] before:list-outside before:list-none before:pointer-events-auto before:absolute before:text-start before:indent-[0px] before:normal-case before:rotate-45 before:visible before:w-1 before:border-r-2 before:border-b-2 before:border-separate before:border-[var(--border)] before:left-[5px] before:top-px before:font-system_ui"
                              />
                              <span className="box-border caret-transparent block grow break-words w-0 ml-2">
                                Ensure they pass fill level and inspection is up
                                to date
                              </span>
                            </label>
                          </li>
                        </ul>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="relative box-border caret-transparent grow border border-[var(--border)] mt-4 rounded-bl rounded-br rounded-tl rounded-tr border-solid">
                  <div className="box-border caret-transparent flex shrink-0 justify-start">
                    <div className="box-border caret-transparent grow-[4] w-full p-4">
                      <div className="items-center box-border caret-transparent flex shrink-0 justify-start">
                        <div className="box-border caret-transparent w-full">
                          <label className="box-border caret-transparent shrink-0 break-words">
                            IDS/ACS
                          </label>
                        </div>
                      </div>
                      <div className="absolute box-border caret-transparent shrink-0 right-2 top-2"></div>
                      <div className="box-border caret-transparent shrink-0 mt-4">
                        <ul className="box-border caret-transparent shrink-0 list-none mt-3.5 pl-0">
                          <li className="box-border caret-transparent shrink-0">
                            <label className="items-center box-border caret-transparent flex shrink-0">
                              <input
                                type="checkbox"
                                value="Disconnect backup batteries for 15 minutes"
                                className="relative text-neutral-600 bg-accent box-border caret-transparent block shrink-0 h-4 w-4 border border-accent p-0 rounded-bl rounded-br rounded-tl rounded-tr border-solid before:accent-auto before:box-border before:caret-transparent before:text-neutral-600 before:block before:shrink-0 before:text-sm before:not-italic before:normal-nums before:font-normal before:h-[9px] before:tracking-[normal] before:leading-[16.8px] before:list-outside before:list-none before:pointer-events-auto before:absolute before:text-start before:indent-[0px] before:normal-case before:rotate-45 before:visible before:w-1 before:border-r-2 before:border-b-2 before:border-separate before:border-[var(--border)] before:left-[5px] before:top-px before:font-system_ui"
                              />
                              <span className="box-border caret-transparent block grow break-words w-0 ml-2">
                                Disconnect backup batteries for 15 minutes
                              </span>
                            </label>
                          </li>
                          <li className="box-border caret-transparent shrink-0 mt-4">
                            <label className="items-center box-border caret-transparent flex shrink-0">
                              <input
                                type="checkbox"
                                value="Clean Bio reader with alcohol."
                                className="relative text-neutral-600 bg-accent box-border caret-transparent block shrink-0 h-4 w-4 border border-accent p-0 rounded-bl rounded-br rounded-tl rounded-tr border-solid before:accent-auto before:box-border before:caret-transparent before:text-neutral-600 before:block before:shrink-0 before:text-sm before:not-italic before:normal-nums before:font-normal before:h-[9px] before:tracking-[normal] before:leading-[16.8px] before:list-outside before:list-none before:pointer-events-auto before:absolute before:text-start before:indent-[0px] before:normal-case before:rotate-45 before:visible before:w-1 before:border-r-2 before:border-b-2 before:border-separate before:border-[var(--border)] before:left-[5px] before:top-px before:font-system_ui"
                              />
                              <span className="box-border caret-transparent block grow break-words w-0 ml-2">
                                Clean Bio reader with alcohol.
                              </span>
                            </label>
                          </li>
                          <li className="box-border caret-transparent shrink-0 mt-4">
                            <label className="items-center box-border caret-transparent flex shrink-0">
                              <input
                                type="checkbox"
                                value="Check Open Options Logs."
                                className="relative text-neutral-600 bg-accent box-border caret-transparent block shrink-0 h-4 w-4 border border-accent p-0 rounded-bl rounded-br rounded-tl rounded-tr border-solid before:accent-auto before:box-border before:caret-transparent before:text-neutral-600 before:block before:shrink-0 before:text-sm before:not-italic before:normal-nums before:font-normal before:h-[9px] before:tracking-[normal] before:leading-[16.8px] before:list-outside before:list-none before:pointer-events-auto before:absolute before:text-start before:indent-[0px] before:normal-case before:rotate-45 before:visible before:w-1 before:border-r-2 before:border-b-2 before:border-separate before:border-[var(--border)] before:left-[5px] before:top-px before:font-system_ui"
                              />
                              <span className="box-border caret-transparent block grow break-words w-0 ml-2">
                                Check Open Options Logs.
                              </span>
                            </label>
                          </li>
                          <li className="box-border caret-transparent shrink-0 mt-4">
                            <label className="items-center box-border caret-transparent flex shrink-0">
                              <input
                                type="checkbox"
                                value="After 15 minutes, check 12V Backup with multimeter to ensure adequate voltage (12v)"
                                className="relative text-neutral-600 bg-accent box-border caret-transparent block shrink-0 h-4 w-4 border border-accent p-0 rounded-bl rounded-br rounded-tl rounded-tr border-solid before:accent-auto before:box-border before:caret-transparent before:text-neutral-600 before:block before:shrink-0 before:text-sm before:not-italic before:normal-nums before:font-normal before:h-[9px] before:tracking-[normal] before:leading-[16.8px] before:list-outside before:list-none before:pointer-events-auto before:absolute before:text-start before:indent-[0px] before:normal-case before:rotate-45 before:visible before:w-1 before:border-r-2 before:border-b-2 before:border-separate before:border-[var(--border)] before:left-[5px] before:top-px before:font-system_ui"
                              />
                              <span className="box-border caret-transparent block grow break-words w-0 ml-2">
                                After 15 minutes, check 12V Backup with
                                multimeter to ensure adequate voltage (12v)
                              </span>
                            </label>
                          </li>
                          <li className="box-border caret-transparent shrink-0 mt-4">
                            <label className="items-center box-border caret-transparent flex shrink-0">
                              <input
                                type="checkbox"
                                value="Visually inspect for forced entry."
                                className="relative text-neutral-600 bg-accent box-border caret-transparent block shrink-0 h-4 w-4 border border-accent p-0 rounded-bl rounded-br rounded-tl rounded-tr border-solid before:accent-auto before:box-border before:caret-transparent before:text-neutral-600 before:block before:shrink-0 before:text-sm before:not-italic before:normal-nums before:font-normal before:h-[9px] before:tracking-[normal] before:leading-[16.8px] before:list-outside before:list-none before:pointer-events-auto before:absolute before:text-start before:indent-[0px] before:normal-case before:rotate-45 before:visible before:w-1 before:border-r-2 before:border-b-2 before:border-separate before:border-[var(--border)] before:left-[5px] before:top-px before:font-system_ui"
                              />
                              <span className="box-border caret-transparent block grow break-words w-0 ml-2">
                                Visually inspect for forced entry.
                              </span>
                            </label>
                          </li>
                        </ul>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="relative box-border caret-transparent grow border border-[var(--border)] mt-4 rounded-bl rounded-br rounded-tl rounded-tr border-solid">
                  <div className="box-border caret-transparent flex shrink-0 justify-start">
                    <div className="box-border caret-transparent grow-[4] w-full p-4">
                      <div className="items-center box-border caret-transparent flex shrink-0 justify-start">
                        <div className="box-border caret-transparent w-full">
                          <label className="box-border caret-transparent shrink-0 break-words">
                            SSF Server Room: Front Photo of Server Rack
                          </label>
                        </div>
                      </div>
                      <div className="absolute box-border caret-transparent shrink-0 right-2 top-2"></div>
                      <div className="box-border caret-transparent shrink-0 mt-4">
                        <div className="box-border caret-transparent shrink-0">
                          <div className="relative box-border caret-transparent shrink-0 mt-4">
                            <div className="box-border caret-transparent shrink-0">
                              <div className="font-semibold box-border caret-transparent shrink-0"></div>
                              <div className="box-border caret-transparent shrink-0 mt-2">
                                <button
                                  type="button"
                                  className="text-slate-400 text-[11.2px] font-medium items-center bg-gray-50 caret-transparent flex flex-col shrink-0 justify-center leading-[13.44px] min-h-[100px] text-center w-full border border-[var(--border)] py-6 rounded-bl rounded-br rounded-tl rounded-tr border-dashed"
                                >
                                  <div className="box-border caret-transparent shrink-0 mb-1">
                                    <img
                                      src="https://c.animaapp.com/mkof8zon8iICvl/assets/icon-53.svg"
                                      alt="Icon"
                                      className="box-border caret-transparent shrink-0 h-5 w-5"
                                    />
                                  </div>
                                  Add Pictures/Files
                                </button>
                                <input
                                  type="file"
                                  className="text-neutral-600 items-baseline bg-transparent box-border caret-transparent hidden shrink-0 text-ellipsis text-nowrap p-0"
                                />
                              </div>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="relative box-border caret-transparent grow border border-[var(--border)] mt-4 rounded-bl rounded-br rounded-tl rounded-tr border-solid">
                  <div className="box-border caret-transparent flex shrink-0 justify-start">
                    <div className="box-border caret-transparent grow-[4] w-full p-4">
                      <div className="items-center box-border caret-transparent flex shrink-0 justify-start">
                        <div className="box-border caret-transparent w-full">
                          <label className="box-border caret-transparent shrink-0 break-words">
                            SSF Server Room: Back Photo of Server Rack
                          </label>
                        </div>
                      </div>
                      <div className="absolute box-border caret-transparent shrink-0 right-2 top-2"></div>
                      <div className="box-border caret-transparent shrink-0 mt-4">
                        <div className="box-border caret-transparent shrink-0">
                          <div className="relative box-border caret-transparent shrink-0 mt-4">
                            <div className="box-border caret-transparent shrink-0">
                              <div className="font-semibold box-border caret-transparent shrink-0"></div>
                              <div className="box-border caret-transparent shrink-0 mt-2">
                                <button
                                  type="button"
                                  className="text-slate-400 text-[11.2px] font-medium items-center bg-gray-50 caret-transparent flex flex-col shrink-0 justify-center leading-[13.44px] min-h-[100px] text-center w-full border border-[var(--border)] py-6 rounded-bl rounded-br rounded-tl rounded-tr border-dashed"
                                >
                                  <div className="box-border caret-transparent shrink-0 mb-1">
                                    <img
                                      src="https://c.animaapp.com/mkof8zon8iICvl/assets/icon-53.svg"
                                      alt="Icon"
                                      className="box-border caret-transparent shrink-0 h-5 w-5"
                                    />
                                  </div>
                                  Add Pictures/Files
                                </button>
                                <input
                                  type="file"
                                  className="text-neutral-600 items-baseline bg-transparent box-border caret-transparent hidden shrink-0 text-ellipsis text-nowrap p-0"
                                />
                              </div>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="relative box-border caret-transparent grow border border-[var(--border)] mt-4 rounded-bl rounded-br rounded-tl rounded-tr border-solid">
                  <div className="box-border caret-transparent flex shrink-0 justify-start">
                    <div className="box-border caret-transparent grow-[4] w-full p-4">
                      <div className="items-center box-border caret-transparent flex shrink-0 justify-start">
                        <div className="box-border caret-transparent w-full">
                          <label className="box-border caret-transparent shrink-0 break-words">
                            SOC Server Room: Front Photo of Server Rack
                          </label>
                        </div>
                      </div>
                      <div className="absolute box-border caret-transparent shrink-0 right-2 top-2"></div>
                      <div className="box-border caret-transparent shrink-0 mt-4">
                        <div className="box-border caret-transparent shrink-0">
                          <div className="relative box-border caret-transparent shrink-0 mt-4">
                            <div className="box-border caret-transparent shrink-0">
                              <div className="font-semibold box-border caret-transparent shrink-0"></div>
                              <div className="box-border caret-transparent shrink-0 mt-2">
                                <button
                                  type="button"
                                  className="text-slate-400 text-[11.2px] font-medium items-center bg-gray-50 caret-transparent flex flex-col shrink-0 justify-center leading-[13.44px] min-h-[100px] text-center w-full border border-[var(--border)] py-6 rounded-bl rounded-br rounded-tl rounded-tr border-dashed"
                                >
                                  <div className="box-border caret-transparent shrink-0 mb-1">
                                    <img
                                      src="https://c.animaapp.com/mkof8zon8iICvl/assets/icon-53.svg"
                                      alt="Icon"
                                      className="box-border caret-transparent shrink-0 h-5 w-5"
                                    />
                                  </div>
                                  Add Pictures/Files
                                </button>
                                <input
                                  type="file"
                                  className="text-neutral-600 items-baseline bg-transparent box-border caret-transparent hidden shrink-0 text-ellipsis text-nowrap p-0"
                                />
                              </div>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="relative box-border caret-transparent grow border border-[var(--border)] mt-4 rounded-bl rounded-br rounded-tl rounded-tr border-solid">
                  <div className="box-border caret-transparent flex shrink-0 justify-start">
                    <div className="box-border caret-transparent grow-[4] w-full p-4">
                      <div className="items-center box-border caret-transparent flex shrink-0 justify-start">
                        <div className="box-border caret-transparent w-full">
                          <label className="box-border caret-transparent shrink-0 break-words">
                            SOC Monitors (IDS, Fiber, CCTV)
                          </label>
                        </div>
                      </div>
                      <div className="absolute box-border caret-transparent shrink-0 right-2 top-2"></div>
                      <div className="text-gray-600 box-border caret-transparent shrink-0 leading-[21px] break-words mt-2">
                        <span className="box-border caret-transparent shrink-0 break-words">
                          <span className="box-border caret-transparent shrink-0 break-words">
                            Provide a photo showing the POV of the 3 SOC
                            monitors
                          </span>
                        </span>
                      </div>
                      <div className="box-border caret-transparent shrink-0 mt-4">
                        <div className="box-border caret-transparent shrink-0">
                          <div className="relative box-border caret-transparent shrink-0 mt-4">
                            <div className="box-border caret-transparent shrink-0">
                              <div className="font-semibold box-border caret-transparent shrink-0"></div>
                              <div className="box-border caret-transparent shrink-0 mt-2">
                                <button
                                  type="button"
                                  className="text-slate-400 text-[11.2px] font-medium items-center bg-gray-50 caret-transparent flex flex-col shrink-0 justify-center leading-[13.44px] min-h-[100px] text-center w-full border border-[var(--border)] py-6 rounded-bl rounded-br rounded-tl rounded-tr border-dashed"
                                >
                                  <div className="box-border caret-transparent shrink-0 mb-1">
                                    <img
                                      src="https://c.animaapp.com/mkof8zon8iICvl/assets/icon-53.svg"
                                      alt="Icon"
                                      className="box-border caret-transparent shrink-0 h-5 w-5"
                                    />
                                  </div>
                                  Add Pictures/Files
                                </button>
                                <input
                                  type="file"
                                  className="text-neutral-600 items-baseline bg-transparent box-border caret-transparent hidden shrink-0 text-ellipsis text-nowrap p-0"
                                />
                              </div>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="relative box-border caret-transparent grow border border-[var(--border)] mt-4 rounded-bl rounded-br rounded-tl rounded-tr border-solid">
                  <div className="box-border caret-transparent flex shrink-0 justify-start">
                    <div className="box-border caret-transparent grow-[4] w-full p-4">
                      <div className="items-center box-border caret-transparent flex shrink-0 justify-start">
                        <div className="box-border caret-transparent w-full">
                          <label className="box-border caret-transparent shrink-0 break-words">
                            SSF Server Room Camera view Monitor (#2 on KVM)
                          </label>
                        </div>
                      </div>
                      <div className="absolute box-border caret-transparent shrink-0 right-2 top-2"></div>
                      <div className="text-gray-600 box-border caret-transparent shrink-0 leading-[21px] break-words mt-2">
                        <span className="box-border caret-transparent shrink-0 break-words">
                          <span className="box-border caret-transparent shrink-0 break-words">
                            Provide a photo showing the KVM monitor on the SSF
                            server room showing monitor #2 (Bosch camera view)
                          </span>
                        </span>
                      </div>
                      <div className="box-border caret-transparent shrink-0 mt-4">
                        <div className="box-border caret-transparent shrink-0">
                          <div className="relative box-border caret-transparent shrink-0 mt-4">
                            <div className="box-border caret-transparent shrink-0">
                              <div className="font-semibold box-border caret-transparent shrink-0"></div>
                              <div className="box-border caret-transparent shrink-0 mt-2">
                                <button
                                  type="button"
                                  className="text-slate-400 text-[11.2px] font-medium items-center bg-gray-50 caret-transparent flex flex-col shrink-0 justify-center leading-[13.44px] min-h-[100px] text-center w-full border border-[var(--border)] py-6 rounded-bl rounded-br rounded-tl rounded-tr border-dashed"
                                >
                                  <div className="box-border caret-transparent shrink-0 mb-1">
                                    <img
                                      src="https://c.animaapp.com/mkof8zon8iICvl/assets/icon-53.svg"
                                      alt="Icon"
                                      className="box-border caret-transparent shrink-0 h-5 w-5"
                                    />
                                  </div>
                                  Add Pictures/Files
                                </button>
                                <input
                                  type="file"
                                  className="text-neutral-600 items-baseline bg-transparent box-border caret-transparent hidden shrink-0 text-ellipsis text-nowrap p-0"
                                />
                              </div>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="relative box-border caret-transparent grow border border-[var(--border)] mt-4 rounded-bl rounded-br rounded-tl rounded-tr border-solid">
                  <div className="box-border caret-transparent flex shrink-0 justify-start">
                    <div className="box-border caret-transparent grow-[4] w-full p-4">
                      <div className="items-center box-border caret-transparent flex shrink-0 justify-start">
                        <div className="box-border caret-transparent w-full">
                          <label className="box-border caret-transparent shrink-0 break-words">
                            Bosch CCTV Inspection
                          </label>
                        </div>
                      </div>
                      <div className="absolute box-border caret-transparent shrink-0 right-2 top-2"></div>
                      <div className="box-border caret-transparent shrink-0 mt-4">
                        <ul className="box-border caret-transparent shrink-0 list-none mt-3.5 pl-0">
                          <li className="box-border caret-transparent shrink-0">
                            <label className="items-center box-border caret-transparent flex shrink-0">
                              <input
                                type="checkbox"
                                value="Visual Inspection Outside"
                                className="relative text-neutral-600 bg-accent box-border caret-transparent block shrink-0 h-4 w-4 border border-accent p-0 rounded-bl rounded-br rounded-tl rounded-tr border-solid before:accent-auto before:box-border before:caret-transparent before:text-neutral-600 before:block before:shrink-0 before:text-sm before:not-italic before:normal-nums before:font-normal before:h-[9px] before:tracking-[normal] before:leading-[16.8px] before:list-outside before:list-none before:pointer-events-auto before:absolute before:text-start before:indent-[0px] before:normal-case before:rotate-45 before:visible before:w-1 before:border-r-2 before:border-b-2 before:border-separate before:border-[var(--border)] before:left-[5px] before:top-px before:font-system_ui"
                              />
                              <span className="box-border caret-transparent block grow break-words w-0 ml-2">
                                Visual Inspection Outside
                              </span>
                            </label>
                          </li>
                          <li className="box-border caret-transparent shrink-0 mt-4">
                            <label className="items-center box-border caret-transparent flex shrink-0">
                              <input
                                type="checkbox"
                                value="Check Camera images from Main Site."
                                className="relative text-neutral-600 bg-accent box-border caret-transparent block shrink-0 h-4 w-4 border border-accent p-0 rounded-bl rounded-br rounded-tl rounded-tr border-solid before:accent-auto before:box-border before:caret-transparent before:text-neutral-600 before:block before:shrink-0 before:text-sm before:not-italic before:normal-nums before:font-normal before:h-[9px] before:tracking-[normal] before:leading-[16.8px] before:list-outside before:list-none before:pointer-events-auto before:absolute before:text-start before:indent-[0px] before:normal-case before:rotate-45 before:visible before:w-1 before:border-r-2 before:border-b-2 before:border-separate before:border-[var(--border)] before:left-[5px] before:top-px before:font-system_ui"
                              />
                              <span className="box-border caret-transparent block grow break-words w-0 ml-2">
                                Check Camera images from Main Site.
                              </span>
                            </label>
                          </li>
                          <li className="box-border caret-transparent shrink-0 mt-4">
                            <label className="items-center box-border caret-transparent flex shrink-0">
                              <input
                                type="checkbox"
                                value="Clean CCTV servers with compressed air."
                                className="relative text-neutral-600 bg-accent box-border caret-transparent block shrink-0 h-4 w-4 border border-accent p-0 rounded-bl rounded-br rounded-tl rounded-tr border-solid before:accent-auto before:box-border before:caret-transparent before:text-neutral-600 before:block before:shrink-0 before:text-sm before:not-italic before:normal-nums before:font-normal before:h-[9px] before:tracking-[normal] before:leading-[16.8px] before:list-outside before:list-none before:pointer-events-auto before:absolute before:text-start before:indent-[0px] before:normal-case before:rotate-45 before:visible before:w-1 before:border-r-2 before:border-b-2 before:border-separate before:border-[var(--border)] before:left-[5px] before:top-px before:font-system_ui"
                              />
                              <span className="box-border caret-transparent block grow break-words w-0 ml-2">
                                Clean CCTV servers with compressed air.
                              </span>
                            </label>
                          </li>
                          <li className="box-border caret-transparent shrink-0 mt-4">
                            <label className="items-center box-border caret-transparent flex shrink-0">
                              <input
                                type="checkbox"
                                value="Check Camera Images from Remote Site (SOC)"
                                className="relative text-neutral-600 bg-accent box-border caret-transparent block shrink-0 h-4 w-4 border border-accent p-0 rounded-bl rounded-br rounded-tl rounded-tr border-solid before:accent-auto before:box-border before:caret-transparent before:text-neutral-600 before:block before:shrink-0 before:text-sm before:not-italic before:normal-nums before:font-normal before:h-[9px] before:tracking-[normal] before:leading-[16.8px] before:list-outside before:list-none before:pointer-events-auto before:absolute before:text-start before:indent-[0px] before:normal-case before:rotate-45 before:visible before:w-1 before:border-r-2 before:border-b-2 before:border-separate before:border-[var(--border)] before:left-[5px] before:top-px before:font-system_ui"
                              />
                              <span className="box-border caret-transparent block grow break-words w-0 ml-2">
                                Check Camera Images from Remote Site (SOC)
                              </span>
                            </label>
                          </li>
                          <li className="box-border caret-transparent shrink-0 mt-4">
                            <label className="items-center box-border caret-transparent flex shrink-0">
                              <input
                                type="checkbox"
                                value="Clean CCTV work station with compressed air."
                                className="relative text-neutral-600 bg-accent box-border caret-transparent block shrink-0 h-4 w-4 border border-accent p-0 rounded-bl rounded-br rounded-tl rounded-tr border-solid before:accent-auto before:box-border before:caret-transparent before:text-neutral-600 before:block before:shrink-0 before:text-sm before:not-italic before:normal-nums before:font-normal before:h-[9px] before:tracking-[normal] before:leading-[16.8px] before:list-outside before:list-none before:pointer-events-auto before:absolute before:text-start before:indent-[0px] before:normal-case before:rotate-45 before:visible before:w-1 before:border-r-2 before:border-b-2 before:border-separate before:border-[var(--border)] before:left-[5px] before:top-px before:font-system_ui"
                              />
                              <span className="box-border caret-transparent block grow break-words w-0 ml-2">
                                Clean CCTV work station with compressed air.
                              </span>
                            </label>
                          </li>
                          <li className="box-border caret-transparent shrink-0 mt-4">
                            <label className="items-center box-border caret-transparent flex shrink-0">
                              <input
                                type="checkbox"
                                value="Check to see if video is recording for 90 days."
                                className="relative text-neutral-600 bg-accent box-border caret-transparent block shrink-0 h-4 w-4 border border-accent p-0 rounded-bl rounded-br rounded-tl rounded-tr border-solid before:accent-auto before:box-border before:caret-transparent before:text-neutral-600 before:block before:shrink-0 before:text-sm before:not-italic before:normal-nums before:font-normal before:h-[9px] before:tracking-[normal] before:leading-[16.8px] before:list-outside before:list-none before:pointer-events-auto before:absolute before:text-start before:indent-[0px] before:normal-case before:rotate-45 before:visible before:w-1 before:border-r-2 before:border-b-2 before:border-separate before:border-[var(--border)] before:left-[5px] before:top-px before:font-system_ui"
                              />
                              <span className="box-border caret-transparent block grow break-words w-0 ml-2">
                                Check to see if video is recording for 90 days.
                              </span>
                            </label>
                          </li>
                        </ul>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="relative box-border caret-transparent grow border border-[var(--border)] mt-4 rounded-bl rounded-br rounded-tl rounded-tr border-solid">
                  <div className="box-border caret-transparent flex shrink-0 justify-start">
                    <div className="box-border caret-transparent grow-[4] w-full p-4">
                      <div className="items-center box-border caret-transparent flex shrink-0 justify-start">
                        <div className="box-border caret-transparent w-full">
                          <label className="box-border caret-transparent shrink-0 break-words">
                            SSF Server room: CCTV Bosch Server (Actual unit)
                            Front Photo POV
                          </label>
                        </div>
                      </div>
                      <div className="absolute box-border caret-transparent shrink-0 right-2 top-2"></div>
                      <div className="text-gray-600 box-border caret-transparent shrink-0 leading-[21px] break-words mt-2">
                        <span className="box-border caret-transparent shrink-0 break-words">
                          <span className="box-border caret-transparent shrink-0 break-words">
                            This is needed to see current state of server (green
                            LED lights on front and back)
                          </span>
                        </span>
                      </div>
                      <div className="box-border caret-transparent shrink-0 mt-4">
                        <div className="box-border caret-transparent shrink-0">
                          <div className="relative box-border caret-transparent shrink-0 mt-4">
                            <div className="box-border caret-transparent shrink-0">
                              <div className="font-semibold box-border caret-transparent shrink-0"></div>
                              <div className="box-border caret-transparent shrink-0 mt-2">
                                <button
                                  type="button"
                                  className="text-slate-400 text-[11.2px] font-medium items-center bg-gray-50 caret-transparent flex flex-col shrink-0 justify-center leading-[13.44px] min-h-[100px] text-center w-full border border-[var(--border)] py-6 rounded-bl rounded-br rounded-tl rounded-tr border-dashed"
                                >
                                  <div className="box-border caret-transparent shrink-0 mb-1">
                                    <img
                                      src="https://c.animaapp.com/mkof8zon8iICvl/assets/icon-53.svg"
                                      alt="Icon"
                                      className="box-border caret-transparent shrink-0 h-5 w-5"
                                    />
                                  </div>
                                  Add Pictures/Files
                                </button>
                                <input
                                  type="file"
                                  className="text-neutral-600 items-baseline bg-transparent box-border caret-transparent hidden shrink-0 text-ellipsis text-nowrap p-0"
                                />
                              </div>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="relative box-border caret-transparent grow border border-[var(--border)] mt-4 rounded-bl rounded-br rounded-tl rounded-tr border-solid">
                  <div className="box-border caret-transparent flex shrink-0 justify-start">
                    <div className="box-border caret-transparent grow-[4] w-full p-4">
                      <div className="items-center box-border caret-transparent flex shrink-0 justify-start">
                        <div className="box-border caret-transparent w-full">
                          <label className="box-border caret-transparent shrink-0 break-words">
                            SSF Server room: CCTV Bosch Server (Actual unit)
                            Back Photo POV
                          </label>
                        </div>
                      </div>
                      <div className="absolute box-border caret-transparent shrink-0 right-2 top-2"></div>
                      <div className="text-gray-600 box-border caret-transparent shrink-0 leading-[21px] break-words mt-2">
                        <span className="box-border caret-transparent shrink-0 break-words">
                          <span className="box-border caret-transparent shrink-0 break-words">
                            This is needed to see current state of server (green
                            LED lights on front and back)
                          </span>
                        </span>
                      </div>
                      <div className="box-border caret-transparent shrink-0 mt-4">
                        <div className="box-border caret-transparent shrink-0">
                          <div className="relative box-border caret-transparent shrink-0 mt-4">
                            <div className="box-border caret-transparent shrink-0">
                              <div className="font-semibold box-border caret-transparent shrink-0"></div>
                              <div className="box-border caret-transparent shrink-0 mt-2">
                                <button
                                  type="button"
                                  className="text-slate-400 text-[11.2px] font-medium items-center bg-gray-50 caret-transparent flex flex-col shrink-0 justify-center leading-[13.44px] min-h-[100px] text-center w-full border border-[var(--border)] py-6 rounded-bl rounded-br rounded-tl rounded-tr border-dashed"
                                >
                                  <div className="box-border caret-transparent shrink-0 mb-1">
                                    <img
                                      src="https://c.animaapp.com/mkof8zon8iICvl/assets/icon-53.svg"
                                      alt="Icon"
                                      className="box-border caret-transparent shrink-0 h-5 w-5"
                                    />
                                  </div>
                                  Add Pictures/Files
                                </button>
                                <input
                                  type="file"
                                  className="text-neutral-600 items-baseline bg-transparent box-border caret-transparent hidden shrink-0 text-ellipsis text-nowrap p-0"
                                />
                              </div>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="relative box-border caret-transparent grow border border-[var(--border)] mt-4 rounded-bl rounded-br rounded-tl rounded-tr border-solid">
                  <div className="box-border caret-transparent flex shrink-0 justify-start">
                    <div className="box-border caret-transparent grow-[4] w-full p-4">
                      <div className="items-center box-border caret-transparent flex shrink-0 justify-start">
                        <div className="box-border caret-transparent w-full">
                          <label className="box-border caret-transparent shrink-0 break-words">
                            SSF Building Flood Lights
                          </label>
                        </div>
                      </div>
                      <div className="absolute box-border caret-transparent shrink-0 right-2 top-2"></div>
                      <div className="text-gray-600 box-border caret-transparent shrink-0 leading-[21px] break-words mt-2">
                        <span className="box-border caret-transparent shrink-0 break-words">
                          <span className="box-border caret-transparent shrink-0 break-words">
                            Check if any lights will need to be replaced
                          </span>
                        </span>
                      </div>
                      <div className="box-border caret-transparent shrink-0 mt-4">
                        <ul className="box-border caret-transparent shrink-0 list-none mt-3.5 pl-0">
                          <li className="box-border caret-transparent shrink-0">
                            <label className="items-center box-border caret-transparent flex shrink-0">
                              <input
                                type="checkbox"
                                value="Visual Inspection"
                                className="relative text-neutral-600 bg-accent box-border caret-transparent block shrink-0 h-4 w-4 border border-accent p-0 rounded-bl rounded-br rounded-tl rounded-tr border-solid before:accent-auto before:box-border before:caret-transparent before:text-neutral-600 before:block before:shrink-0 before:text-sm before:not-italic before:normal-nums before:font-normal before:h-[9px] before:tracking-[normal] before:leading-[16.8px] before:list-outside before:list-none before:pointer-events-auto before:absolute before:text-start before:indent-[0px] before:normal-case before:rotate-45 before:visible before:w-1 before:border-r-2 before:border-b-2 before:border-separate before:border-[var(--border)] before:left-[5px] before:top-px before:font-system_ui"
                              />
                              <span className="box-border caret-transparent block grow break-words w-0 ml-2">
                                Visual Inspection
                              </span>
                            </label>
                          </li>
                          <li className="box-border caret-transparent shrink-0 mt-4">
                            <label className="items-center box-border caret-transparent flex shrink-0">
                              <input
                                type="checkbox"
                                value="Check Photo sensors."
                                className="relative text-neutral-600 bg-accent box-border caret-transparent block shrink-0 h-4 w-4 border border-accent p-0 rounded-bl rounded-br rounded-tl rounded-tr border-solid before:accent-auto before:box-border before:caret-transparent before:text-neutral-600 before:block before:shrink-0 before:text-sm before:not-italic before:normal-nums before:font-normal before:h-[9px] before:tracking-[normal] before:leading-[16.8px] before:list-outside before:list-none before:pointer-events-auto before:absolute before:text-start before:indent-[0px] before:normal-case before:rotate-45 before:visible before:w-1 before:border-r-2 before:border-b-2 before:border-separate before:border-[var(--border)] before:left-[5px] before:top-px before:font-system_ui"
                              />
                              <span className="box-border caret-transparent block grow break-words w-0 ml-2">
                                Check Photo sensors.
                              </span>
                            </label>
                          </li>
                        </ul>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="relative box-border caret-transparent grow border border-[var(--border)] mt-4 rounded-bl rounded-br rounded-tl rounded-tr border-solid">
                  <div className="box-border caret-transparent flex shrink-0 justify-start">
                    <div className="box-border caret-transparent grow-[4] w-full p-4">
                      <div className="items-center box-border caret-transparent flex shrink-0 justify-start">
                        <div className="box-border caret-transparent w-full">
                          <label className="box-border caret-transparent shrink-0 break-words">
                            UPS
                          </label>
                        </div>
                      </div>
                      <div className="absolute box-border caret-transparent shrink-0 right-2 top-2"></div>
                      <div className="text-gray-600 box-border caret-transparent shrink-0 leading-[21px] break-words mt-2">
                        <span className="box-border caret-transparent shrink-0 break-words">
                          <span className="box-border caret-transparent shrink-0 break-words">
                            Ensure UPS starts once power is cut from breaker
                            (Audible sound + LCD display will turn from Blue
                            -&gt; Orange, indicating it is using batteries)
                          </span>
                        </span>
                      </div>
                      <div className="box-border caret-transparent shrink-0 mt-4">
                        <ul className="box-border caret-transparent shrink-0 list-none mt-3.5 pl-0">
                          <li className="box-border caret-transparent shrink-0">
                            <label className="items-center box-border caret-transparent flex shrink-0">
                              <input
                                type="checkbox"
                                value="Check for alarms"
                                className="relative text-neutral-600 bg-accent box-border caret-transparent block shrink-0 h-4 w-4 border border-accent p-0 rounded-bl rounded-br rounded-tl rounded-tr border-solid before:accent-auto before:box-border before:caret-transparent before:text-neutral-600 before:block before:shrink-0 before:text-sm before:not-italic before:normal-nums before:font-normal before:h-[9px] before:tracking-[normal] before:leading-[16.8px] before:list-outside before:list-none before:pointer-events-auto before:absolute before:text-start before:indent-[0px] before:normal-case before:rotate-45 before:visible before:w-1 before:border-r-2 before:border-b-2 before:border-separate before:border-[var(--border)] before:left-[5px] before:top-px before:font-system_ui"
                              />
                              <span className="box-border caret-transparent block grow break-words w-0 ml-2">
                                Check for alarms
                              </span>
                            </label>
                          </li>
                          <li className="box-border caret-transparent shrink-0 mt-4">
                            <label className="items-center box-border caret-transparent flex shrink-0">
                              <input
                                type="checkbox"
                                value="Check for charging"
                                className="relative text-neutral-600 bg-accent box-border caret-transparent block shrink-0 h-4 w-4 border border-accent p-0 rounded-bl rounded-br rounded-tl rounded-tr border-solid before:accent-auto before:box-border before:caret-transparent before:text-neutral-600 before:block before:shrink-0 before:text-sm before:not-italic before:normal-nums before:font-normal before:h-[9px] before:tracking-[normal] before:leading-[16.8px] before:list-outside before:list-none before:pointer-events-auto before:absolute before:text-start before:indent-[0px] before:normal-case before:rotate-45 before:visible before:w-1 before:border-r-2 before:border-b-2 before:border-separate before:border-[var(--border)] before:left-[5px] before:top-px before:font-system_ui"
                              />
                              <span className="box-border caret-transparent block grow break-words w-0 ml-2">
                                Check for charging
                              </span>
                            </label>
                          </li>
                          <li className="box-border caret-transparent shrink-0 mt-4">
                            <label className="items-center box-border caret-transparent flex shrink-0">
                              <input
                                type="checkbox"
                                value="Test by cutting power to rack via breaker."
                                className="relative text-neutral-600 bg-accent box-border caret-transparent block shrink-0 h-4 w-4 border border-accent p-0 rounded-bl rounded-br rounded-tl rounded-tr border-solid before:accent-auto before:box-border before:caret-transparent before:text-neutral-600 before:block before:shrink-0 before:text-sm before:not-italic before:normal-nums before:font-normal before:h-[9px] before:tracking-[normal] before:leading-[16.8px] before:list-outside before:list-none before:pointer-events-auto before:absolute before:text-start before:indent-[0px] before:normal-case before:rotate-45 before:visible before:w-1 before:border-r-2 before:border-b-2 before:border-separate before:border-[var(--border)] before:left-[5px] before:top-px before:font-system_ui"
                              />
                              <span className="box-border caret-transparent block grow break-words w-0 ml-2">
                                Test by cutting power to rack via breaker.
                              </span>
                            </label>
                          </li>
                        </ul>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="relative box-border caret-transparent grow border border-[var(--border)] mt-4 rounded-bl rounded-br rounded-tl rounded-tr border-solid">
                  <div className="box-border caret-transparent flex shrink-0 justify-start">
                    <div className="box-border caret-transparent grow-[4] w-full p-4">
                      <div className="items-center box-border caret-transparent flex shrink-0 justify-start">
                        <div className="box-border caret-transparent w-full">
                          <label className="box-border caret-transparent shrink-0 break-words">
                            CyberSecure
                          </label>
                        </div>
                      </div>
                      <div className="absolute box-border caret-transparent shrink-0 right-2 top-2"></div>
                      <div className="text-gray-600 box-border caret-transparent shrink-0 leading-[21px] break-words mt-2">
                        <span className="box-border caret-transparent shrink-0 break-words">
                          <span className="box-border caret-transparent shrink-0 break-words">
                            Quick glance on #3 on KVM monitor to check normal
                            operation status (All green dots on map)
                          </span>
                        </span>
                      </div>
                      <div className="box-border caret-transparent shrink-0 mt-4">
                        <ul className="box-border caret-transparent shrink-0 list-none mt-3.5 pl-0">
                          <li className="box-border caret-transparent shrink-0">
                            <label className="items-center box-border caret-transparent flex shrink-0">
                              <input
                                type="checkbox"
                                value="Visual Inspection"
                                className="relative text-neutral-600 bg-accent box-border caret-transparent block shrink-0 h-4 w-4 border border-accent p-0 rounded-bl rounded-br rounded-tl rounded-tr border-solid before:accent-auto before:box-border before:caret-transparent before:text-neutral-600 before:block before:shrink-0 before:text-sm before:not-italic before:normal-nums before:font-normal before:h-[9px] before:tracking-[normal] before:leading-[16.8px] before:list-outside before:list-none before:pointer-events-auto before:absolute before:text-start before:indent-[0px] before:normal-case before:rotate-45 before:visible before:w-1 before:border-r-2 before:border-b-2 before:border-separate before:border-[var(--border)] before:left-[5px] before:top-px before:font-system_ui"
                              />
                              <span className="box-border caret-transparent block grow break-words w-0 ml-2">
                                Visual Inspection
                              </span>
                            </label>
                          </li>
                          <li className="box-border caret-transparent shrink-0 mt-4">
                            <label className="items-center box-border caret-transparent flex shrink-0">
                              <input
                                type="checkbox"
                                value="Check Log for Errors/Alarms (Also Vanguard)"
                                className="relative text-neutral-600 bg-accent box-border caret-transparent block shrink-0 h-4 w-4 border border-accent p-0 rounded-bl rounded-br rounded-tl rounded-tr border-solid before:accent-auto before:box-border before:caret-transparent before:text-neutral-600 before:block before:shrink-0 before:text-sm before:not-italic before:normal-nums before:font-normal before:h-[9px] before:tracking-[normal] before:leading-[16.8px] before:list-outside before:list-none before:pointer-events-auto before:absolute before:text-start before:indent-[0px] before:normal-case before:rotate-45 before:visible before:w-1 before:border-r-2 before:border-b-2 before:border-separate before:border-[var(--border)] before:left-[5px] before:top-px before:font-system_ui"
                              />
                              <span className="box-border caret-transparent block grow break-words w-0 ml-2">
                                Check Log for Errors/Alarms (Also Vanguard)
                              </span>
                            </label>
                          </li>
                        </ul>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <h2 className="text-[21px] font-semibold items-center box-border caret-transparent flex shrink-0 leading-[25.2px] break-words mt-6">
            <img
              src="https://c.animaapp.com/mkof8zon8iICvl/assets/icon-52.svg"
              alt="Icon"
              className="box-border caret-transparent shrink-0 h-[7px] w-3 -scale-100"
            />
            <span className="box-border caret-transparent block basis-[0%] grow break-words ml-2">
              Issues or Problems
            </span>
          </h2>
          <div className="box-border caret-transparent shrink-0 h-[806px]">
            <div className="box-border caret-transparent flex flex-col shrink-0">
              <div className="box-border caret-transparent shrink-0">
                <div className="relative box-border caret-transparent grow border border-[var(--border)] mt-4 rounded-bl rounded-br rounded-tl rounded-tr border-solid">
                  <div className="box-border caret-transparent flex shrink-0 justify-start">
                    <div className="box-border caret-transparent grow-[4] w-full p-4">
                      <div className="items-center box-border caret-transparent flex shrink-0 justify-start">
                        <div className="box-border caret-transparent w-full">
                          <label className="box-border caret-transparent shrink-0 break-words">
                            Failure Detected During:
                          </label>
                        </div>
                      </div>
                      <div className="absolute box-border caret-transparent shrink-0 right-2 top-2"></div>
                      <div className="box-border caret-transparent shrink-0 mt-4">
                        <ul className="box-border caret-transparent shrink-0 list-none mt-3.5 pl-0">
                          <li className="box-border caret-transparent shrink-0">
                            <label className="items-center box-border caret-transparent flex shrink-0">
                              <input
                                type="radio"
                                value="Scheduled Maintenance"
                                className="relative text-neutral-600 box-border caret-transparent block shrink-0 h-4 w-4 border-[var(--border)] p-0 rounded-[50%] border-solid"
                              />
                              <span className="box-border caret-transparent block grow break-words w-0 ml-2">
                                Scheduled Maintenance
                              </span>
                            </label>
                          </li>
                          <li className="box-border caret-transparent shrink-0 mt-4">
                            <label className="items-center box-border caret-transparent flex shrink-0">
                              <input
                                type="radio"
                                value="Handling"
                                className="relative text-neutral-600 box-border caret-transparent block shrink-0 h-4 w-4 border-[var(--border)] p-0 rounded-[50%] border-solid"
                              />
                              <span className="box-border caret-transparent block grow break-words w-0 ml-2">
                                Handling
                              </span>
                            </label>
                          </li>
                          <li className="box-border caret-transparent shrink-0 mt-4">
                            <label className="items-center box-border caret-transparent flex shrink-0">
                              <input
                                type="radio"
                                value="Normal Operation"
                                className="relative text-neutral-600 box-border caret-transparent block shrink-0 h-4 w-4 border-[var(--border)] p-0 rounded-[50%] border-solid"
                              />
                              <span className="box-border caret-transparent block grow break-words w-0 ml-2">
                                Normal Operation
                              </span>
                            </label>
                          </li>
                          <li className="box-border caret-transparent shrink-0 mt-4">
                            <label className="items-center box-border caret-transparent flex shrink-0">
                              <input
                                type="radio"
                                value="Test"
                                className="relative text-neutral-600 box-border caret-transparent block shrink-0 h-4 w-4 border-[var(--border)] p-0 rounded-[50%] border-solid"
                              />
                              <span className="box-border caret-transparent block grow break-words w-0 ml-2">
                                Test
                              </span>
                            </label>
                          </li>
                          <li className="box-border caret-transparent shrink-0 mt-4">
                            <label className="items-center box-border caret-transparent flex shrink-0">
                              <input
                                type="radio"
                                value="Storage"
                                className="relative text-neutral-600 box-border caret-transparent block shrink-0 h-4 w-4 border-[var(--border)] p-0 rounded-[50%] border-solid"
                              />
                              <span className="box-border caret-transparent block grow break-words w-0 ml-2">
                                Storage
                              </span>
                            </label>
                          </li>
                          <li className="box-border caret-transparent shrink-0 mt-4">
                            <label className="items-center box-border caret-transparent flex shrink-0">
                              <input
                                type="radio"
                                value="Inspection"
                                className="relative text-neutral-600 box-border caret-transparent block shrink-0 h-4 w-4 border-[var(--border)] p-0 rounded-[50%] border-solid"
                              />
                              <span className="box-border caret-transparent block grow break-words w-0 ml-2">
                                Inspection
                              </span>
                            </label>
                          </li>
                          <li className="box-border caret-transparent shrink-0 mt-4">
                            <label className="items-center box-border caret-transparent flex shrink-0">
                              <input
                                type="radio"
                                value="Other"
                                className="relative text-neutral-600 box-border caret-transparent block shrink-0 h-4 w-4 border-[var(--border)] p-0 rounded-[50%] border-solid"
                              />
                              <span className="box-border caret-transparent block grow break-words w-0 ml-2">
                                Other
                              </span>
                            </label>
                          </li>
                        </ul>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="relative box-border caret-transparent grow border border-[var(--border)] mt-4 rounded-bl rounded-br rounded-tl rounded-tr border-solid">
                  <div className="box-border caret-transparent flex shrink-0 justify-start">
                    <div className="box-border caret-transparent grow-[4] w-full p-4">
                      <div className="items-center box-border caret-transparent flex shrink-0 justify-start">
                        <div className="box-border caret-transparent w-full">
                          <label className="box-border caret-transparent shrink-0 break-words">
                            First Indication of Trouble:
                          </label>
                        </div>
                      </div>
                      <div className="absolute box-border caret-transparent shrink-0 right-2 top-2"></div>
                      <div className="box-border caret-transparent shrink-0 mt-4">
                        <ul className="box-border caret-transparent shrink-0 list-none mt-3.5 pl-0">
                          <li className="box-border caret-transparent shrink-0">
                            <label className="items-center box-border caret-transparent flex shrink-0">
                              <input
                                type="radio"
                                value="Inoperative"
                                className="relative text-neutral-600 box-border caret-transparent block shrink-0 h-4 w-4 border-[var(--border)] p-0 rounded-[50%] border-solid"
                              />
                              <span className="box-border caret-transparent block grow break-words w-0 ml-2">
                                Inoperative
                              </span>
                            </label>
                          </li>
                          <li className="box-border caret-transparent shrink-0 mt-4">
                            <label className="items-center box-border caret-transparent flex shrink-0">
                              <input
                                type="radio"
                                value="Overheating"
                                className="relative text-neutral-600 box-border caret-transparent block shrink-0 h-4 w-4 border-[var(--border)] p-0 rounded-[50%] border-solid"
                              />
                              <span className="box-border caret-transparent block grow break-words w-0 ml-2">
                                Overheating
                              </span>
                            </label>
                          </li>
                          <li className="box-border caret-transparent shrink-0 mt-4">
                            <label className="items-center box-border caret-transparent flex shrink-0">
                              <input
                                type="radio"
                                value="Out of Adjustment"
                                className="relative text-neutral-600 box-border caret-transparent block shrink-0 h-4 w-4 border-[var(--border)] p-0 rounded-[50%] border-solid"
                              />
                              <span className="box-border caret-transparent block grow break-words w-0 ml-2">
                                Out of Adjustment
                              </span>
                            </label>
                          </li>
                          <li className="box-border caret-transparent shrink-0 mt-4">
                            <label className="items-center box-border caret-transparent flex shrink-0">
                              <input
                                type="radio"
                                value="Noise"
                                className="relative text-neutral-600 box-border caret-transparent block shrink-0 h-4 w-4 border-[var(--border)] p-0 rounded-[50%] border-solid"
                              />
                              <span className="box-border caret-transparent block grow break-words w-0 ml-2">
                                Noise
                              </span>
                            </label>
                          </li>
                          <li className="box-border caret-transparent shrink-0 mt-4">
                            <label className="items-center box-border caret-transparent flex shrink-0">
                              <input
                                type="radio"
                                value="Low Performance"
                                className="relative text-neutral-600 box-border caret-transparent block shrink-0 h-4 w-4 border-[var(--border)] p-0 rounded-[50%] border-solid"
                              />
                              <span className="box-border caret-transparent block grow break-words w-0 ml-2">
                                Low Performance
                              </span>
                            </label>
                          </li>
                          <li className="box-border caret-transparent shrink-0 mt-4">
                            <label className="items-center box-border caret-transparent flex shrink-0">
                              <input
                                type="radio"
                                value="Other"
                                className="relative text-neutral-600 box-border caret-transparent block shrink-0 h-4 w-4 border-[var(--border)] p-0 rounded-[50%] border-solid"
                              />
                              <span className="box-border caret-transparent block grow break-words w-0 ml-2">
                                Other
                              </span>
                            </label>
                          </li>
                        </ul>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="relative box-border caret-transparent grow border border-[var(--border)] mt-4 rounded-bl rounded-br rounded-tl rounded-tr border-solid">
                  <div className="box-border caret-transparent flex shrink-0 justify-start">
                    <div className="box-border caret-transparent grow-[4] w-full p-4">
                      <div className="items-center box-border caret-transparent flex shrink-0 justify-start">
                        <div className="box-border caret-transparent w-full">
                          <label className="box-border caret-transparent shrink-0 break-words">
                            Describe issue, troubleshooting, current state and
                            next steps
                          </label>
                        </div>
                      </div>
                      <div className="absolute box-border caret-transparent shrink-0 right-2 top-2"></div>
                      <div className="box-border caret-transparent shrink-0 mt-4">
                        <div className="box-border caret-transparent flex flex-col shrink-0 gap-y-0">
                          <div className="self-start box-border caret-transparent shrink-0"></div>
                          <div className="box-border caret-transparent gap-x-2 flex flex-col shrink-0 gap-y-2">
                            <textarea
                              placeholder="Enter Text"
                              className="bg-transparent box-border caret-transparent flex shrink-0 h-10 tracking-[-0.2px] leading-[20.0004px] max-h-[220px] min-h-10 w-full border-[var(--border)] px-3 py-[9px] rounded-bl rounded-br rounded-tl rounded-tr"
                            >
                              None
                            </textarea>
                          </div>
                        </div>
                        <div className="items-center box-border caret-transparent flex shrink-0"></div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="relative box-border caret-transparent grow border border-[var(--border)] mt-4 rounded-bl rounded-br rounded-tl rounded-tr border-solid">
                  <div className="box-border caret-transparent flex shrink-0 justify-start">
                    <div className="box-border caret-transparent grow-[4] w-full p-4">
                      <div className="items-center box-border caret-transparent flex shrink-0 justify-start">
                        <div className="box-border caret-transparent w-full">
                          <label className="box-border caret-transparent shrink-0 break-words">
                            Comments or Remarks: (Include Date/Time)
                          </label>
                        </div>
                      </div>
                      <div className="absolute box-border caret-transparent shrink-0 right-2 top-2"></div>
                      <div className="box-border caret-transparent shrink-0 mt-4">
                        <div className="box-border caret-transparent flex flex-col shrink-0 gap-y-0">
                          <div className="self-start box-border caret-transparent shrink-0"></div>
                          <div className="box-border caret-transparent gap-x-2 flex flex-col shrink-0 gap-y-2">
                            <textarea
                              placeholder="Enter Text"
                              className="bg-transparent box-border caret-transparent flex shrink-0 h-10 tracking-[-0.2px] leading-[20.0004px] max-h-[220px] min-h-10 w-full border-[var(--border)] px-3 py-[9px] rounded-bl rounded-br rounded-tl rounded-tr"
                            >
                              None
                            </textarea>
                          </div>
                        </div>
                        <div className="items-center box-border caret-transparent flex shrink-0"></div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <h2 className="text-[21px] font-semibold items-center box-border caret-transparent flex shrink-0 leading-[25.2px] break-words mt-6">
            <img
              src="https://c.animaapp.com/mkof8zon8iICvl/assets/icon-52.svg"
              alt="Icon"
              className="box-border caret-transparent shrink-0 h-[7px] w-3 -scale-100"
            />
            <span className="box-border caret-transparent block basis-[0%] grow break-words ml-2">
              Outstanding Items or Customer Comments:
            </span>
          </h2>
          <div className="box-border caret-transparent shrink-0 h-[246px]">
            <div className="box-border caret-transparent flex flex-col shrink-0">
              <div className="box-border caret-transparent shrink-0">
                <div className="relative box-border caret-transparent grow border border-[var(--border)] mt-4 rounded-bl rounded-br rounded-tl rounded-tr border-solid">
                  <div className="box-border caret-transparent flex shrink-0 justify-start">
                    <div className="box-border caret-transparent grow-[4] w-full p-4">
                      <div className="items-center box-border caret-transparent flex shrink-0 justify-start">
                        <div className="box-border caret-transparent w-full">
                          <label className="box-border caret-transparent shrink-0 break-words">
                            Outstanding Items:
                          </label>
                        </div>
                      </div>
                      <div className="absolute box-border caret-transparent shrink-0 right-2 top-2"></div>
                      <div className="box-border caret-transparent shrink-0 mt-4">
                        <div className="box-border caret-transparent flex flex-col shrink-0 gap-y-0">
                          <div className="self-start box-border caret-transparent shrink-0"></div>
                          <div className="box-border caret-transparent gap-x-2 flex flex-col shrink-0 gap-y-2">
                            <textarea
                              placeholder="Enter Text"
                              className="bg-transparent box-border caret-transparent flex shrink-0 h-10 tracking-[-0.2px] leading-[20.0004px] max-h-[220px] min-h-10 w-full border-[var(--border)] px-3 py-[9px] rounded-bl rounded-br rounded-tl rounded-tr"
                            >
                              None
                            </textarea>
                          </div>
                        </div>
                        <div className="items-center box-border caret-transparent flex shrink-0"></div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="relative box-border caret-transparent grow border border-[var(--border)] mt-4 rounded-bl rounded-br rounded-tl rounded-tr border-solid">
                  <div className="box-border caret-transparent flex shrink-0 justify-start">
                    <div className="box-border caret-transparent grow-[4] w-full p-4">
                      <div className="items-center box-border caret-transparent flex shrink-0 justify-start">
                        <div className="box-border caret-transparent w-full">
                          <label className="box-border caret-transparent shrink-0 break-words">
                            Customer Comments:
                          </label>
                        </div>
                      </div>
                      <div className="absolute box-border caret-transparent shrink-0 right-2 top-2"></div>
                      <div className="box-border caret-transparent shrink-0 mt-4">
                        <div className="box-border caret-transparent flex flex-col shrink-0 gap-y-0">
                          <div className="self-start box-border caret-transparent shrink-0"></div>
                          <div className="box-border caret-transparent gap-x-2 flex flex-col shrink-0 gap-y-2">
                            <textarea
                              placeholder="Enter Text"
                              className="bg-transparent box-border caret-transparent flex shrink-0 h-10 tracking-[-0.2px] leading-[20.0004px] max-h-[220px] min-h-10 w-full border-[var(--border)] px-3 py-[9px] rounded-bl rounded-br rounded-tl rounded-tr"
                            >
                              None
                            </textarea>
                          </div>
                        </div>
                        <div className="items-center box-border caret-transparent flex shrink-0"></div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <h2 className="text-[21px] font-semibold items-center box-border caret-transparent flex shrink-0 leading-[25.2px] break-words mt-6">
            <img
              src="https://c.animaapp.com/mkof8zon8iICvl/assets/icon-52.svg"
              alt="Icon"
              className="box-border caret-transparent shrink-0 h-[7px] w-3 -scale-100"
            />
            <span className="box-border caret-transparent block basis-[0%] grow break-words ml-2">
              Review &amp; Sign
            </span>
          </h2>
          <div className="box-border caret-transparent shrink-0 h-[710px]">
            <div className="box-border caret-transparent flex flex-col shrink-0">
              <div className="box-border caret-transparent shrink-0">
                <div className="relative box-border caret-transparent grow border border-[var(--border)] mt-4 rounded-bl rounded-br rounded-tl rounded-tr border-solid">
                  <div className="box-border caret-transparent flex shrink-0 justify-start">
                    <div className="box-border caret-transparent grow-[4] w-full p-4">
                      <div className="items-center box-border caret-transparent flex shrink-0 justify-start">
                        <div className="box-border caret-transparent w-full">
                          <label className="box-border caret-transparent shrink-0 break-words after:accent-auto after:box-border after:caret-transparent after:text-red-500 after:shrink-0 after:text-sm after:not-italic after:normal-nums after:font-normal after:tracking-[normal] after:leading-[16.8px] after:list-outside after:list-disc after:break-words after:pointer-events-auto after:text-start after:indent-[0px] after:normal-case after:visible after:border-separate after:font-system_ui">
                            SSF Maintenance Manager Signature:
                          </label>
                        </div>
                      </div>
                      <div className="absolute box-border caret-transparent shrink-0 right-2 top-2"></div>
                      <div className="box-border caret-transparent shrink-0 mt-4">
                        <div className="box-border caret-transparent shrink-0">
                          <div className="box-border caret-transparent shrink-0">
                            <button
                              type="button"
                              className="text-slate-400 text-[12.6px] font-semibold items-center bg-gray-50 caret-transparent flex flex-col shrink-0 justify-center leading-[15.12px] min-h-[100px] text-center w-full border border-[var(--border)] mb-2 rounded-bl rounded-br rounded-tl rounded-tr border-solid"
                            >
                              Click here to sign
                            </button>
                          </div>
                        </div>
                      </div>
                      <div className="items-center box-border caret-transparent shrink-0 justify-between pt-2">
                        <div className="text-gray-600 text-[12.6px] box-border caret-transparent shrink-0 leading-[15.12px] mt-2">
                          Signed by{" "}
                          <span className="text-sm font-semibold box-border caret-transparent shrink-0 leading-[16.8px]">
                            Pedro Modesto
                          </span>
                          on{" "}
                          <span className="text-sm font-semibold box-border caret-transparent shrink-0 leading-[16.8px]">
                            21/01/2026, 12:20
                          </span>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="relative box-border caret-transparent grow border border-[var(--border)] mt-4 rounded-bl rounded-br rounded-tl rounded-tr border-solid">
                  <div className="box-border caret-transparent flex shrink-0 justify-start">
                    <div className="box-border caret-transparent grow-[4] w-full p-4">
                      <div className="items-center box-border caret-transparent flex shrink-0 justify-start">
                        <div className="box-border caret-transparent w-full">
                          <label className="box-border caret-transparent shrink-0 break-words after:accent-auto after:box-border after:caret-transparent after:text-red-500 after:shrink-0 after:text-sm after:not-italic after:normal-nums after:font-normal after:tracking-[normal] after:leading-[16.8px] after:list-outside after:list-disc after:break-words after:pointer-events-auto after:text-start after:indent-[0px] after:normal-case after:visible after:border-separate after:font-system_ui">
                            SSF Maintenance Manager Name:
                          </label>
                        </div>
                      </div>
                      <div className="absolute box-border caret-transparent shrink-0 right-2 top-2"></div>
                      <div className="box-border caret-transparent shrink-0 mt-4">
                        <div className="box-border caret-transparent flex flex-col shrink-0 gap-y-0">
                          <div className="self-start box-border caret-transparent shrink-0"></div>
                          <div className="box-border caret-transparent gap-x-2 flex flex-col shrink-0 gap-y-2">
                            <textarea
                              placeholder="Enter Text"
                              className="bg-transparent box-border caret-transparent flex shrink-0 h-10 tracking-[-0.2px] leading-[20.0004px] max-h-[220px] min-h-10 w-full border-[var(--border)] px-3 py-[9px] rounded-bl rounded-br rounded-tl rounded-tr"
                            >
                              Pedro Modesto
                            </textarea>
                          </div>
                        </div>
                        <div className="items-center box-border caret-transparent flex shrink-0"></div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="relative box-border caret-transparent grow border border-[var(--border)] mt-4 rounded-bl rounded-br rounded-tl rounded-tr border-solid">
                  <div className="box-border caret-transparent flex shrink-0 justify-start">
                    <div className="box-border caret-transparent grow-[4] w-full p-4">
                      <div className="items-center box-border caret-transparent flex shrink-0 justify-start">
                        <div className="box-border caret-transparent w-full">
                          <label className="box-border caret-transparent shrink-0 break-words after:accent-auto after:box-border after:caret-transparent after:text-red-500 after:shrink-0 after:text-sm after:not-italic after:normal-nums after:font-normal after:tracking-[normal] after:leading-[16.8px] after:list-outside after:list-disc after:break-words after:pointer-events-auto after:text-start after:indent-[0px] after:normal-case after:visible after:border-separate after:font-system_ui">
                            SSF Site Lead Signature:
                          </label>
                        </div>
                      </div>
                      <div className="absolute box-border caret-transparent shrink-0 right-2 top-2"></div>
                      <div className="text-gray-600 box-border caret-transparent shrink-0 leading-[21px] break-words mt-2">
                        <span className="box-border caret-transparent shrink-0 break-words">
                          <span className="box-border caret-transparent shrink-0 break-words">
                            This form can be signed and completed by Maintenance
                            Manager in case Site Lead is on PTO
                          </span>
                        </span>
                      </div>
                      <div className="box-border caret-transparent shrink-0 mt-4">
                        <div className="box-border caret-transparent shrink-0">
                          <div className="box-border caret-transparent shrink-0">
                            <button
                              type="button"
                              className="text-slate-400 text-[12.6px] font-semibold items-center bg-gray-50 caret-transparent flex flex-col shrink-0 justify-center leading-[15.12px] min-h-[100px] text-center w-full border border-[var(--border)] mb-2 rounded-bl rounded-br rounded-tl rounded-tr border-solid"
                            >
                              Click here to sign
                            </button>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="relative box-border caret-transparent grow border border-[var(--border)] mt-4 rounded-bl rounded-br rounded-tl rounded-tr border-solid">
                  <div className="box-border caret-transparent flex shrink-0 justify-start">
                    <div className="box-border caret-transparent grow-[4] w-full p-4">
                      <div className="items-center box-border caret-transparent flex shrink-0 justify-start">
                        <div className="box-border caret-transparent w-full">
                          <label className="box-border caret-transparent shrink-0 break-words after:accent-auto after:box-border after:caret-transparent after:text-red-500 after:shrink-0 after:text-sm after:not-italic after:normal-nums after:font-normal after:tracking-[normal] after:leading-[16.8px] after:list-outside after:list-disc after:break-words after:pointer-events-auto after:text-start after:indent-[0px] after:normal-case after:visible after:border-separate after:font-system_ui">
                            SSF Site Lead Name:
                          </label>
                        </div>
                      </div>
                      <div className="absolute box-border caret-transparent shrink-0 right-2 top-2"></div>
                      <div className="box-border caret-transparent shrink-0 mt-4">
                        <div className="box-border caret-transparent flex flex-col shrink-0 gap-y-0">
                          <div className="self-start box-border caret-transparent shrink-0"></div>
                          <div className="box-border caret-transparent gap-x-2 flex flex-col shrink-0 gap-y-2">
                            <textarea
                              placeholder="Enter Text"
                              className="bg-transparent box-border caret-transparent flex shrink-0 h-10 tracking-[-0.2px] leading-[20.0004px] max-h-[220px] min-h-10 w-full border-[var(--border)] px-3 py-[9px] rounded-bl rounded-br rounded-tl rounded-tr"
                            >
                              Jason Degg
                            </textarea>
                          </div>
                        </div>
                        <div className="items-center box-border caret-transparent flex shrink-0"></div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};
