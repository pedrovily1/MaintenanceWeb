    ...
    <div className="items-center box-border caret-transparent gap-x-2 flex shrink-0 gap-y-2 relative">
      <div className="items-center box-border caret-transparent flex shrink-0">
        <button
          ...
          className={`items-center bg-white caret-transparent flex h-8 justify-center tracking-[-0.2px] leading-[20.0004px] text-center border ${assignedTo ? 'border-blue-500 bg-sky-50' : 'border-zinc-200'} overflow-hidden px-1 rounded-bl rounded-br rounded-tl rounded-tr border-solid hover:border-neutral-300`}
        >
        ...
        {showUserDropdown && (
          <div className="absolute top-10 left-0 z-50 bg-white border border-zinc-200 rounded shadow-lg min-w-[150px] py-1">
            <div 
            ...
            {allUsers.map(user => (
              <div 
                key={user.id}
                className="px-3 py-2 hover:bg-gray-100 cursor-pointer text-sm border-t border-zinc-50 flex items-center gap-2"
